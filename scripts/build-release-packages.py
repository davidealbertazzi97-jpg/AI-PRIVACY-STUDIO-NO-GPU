...
shutil.copy(ROOT / "packaging" / "INSTALLER_NOTICE.en-it.txt", appdir / "INSTALLER_NOTICE.en-it.txt")
shutil.copy(ROOT / "packaging" / "linux" / "ai-privacy-studio.desktop", appdir / "ai-privacy-studio.desktop")
...
with tarfile.open(linux_tar, "w:gz") as tar:
    tar.add(appdir, arcname=f"AI-Privacy-Studio-v{VERSION}")
    ...
    for rel_path in TRACKED_PATHS:
        ...
        if full_p.is_file():
            ...
            for sub in full_p.rglob("*"):
                if sub.is_file():
                    zf.write(sub, f"AI-Privacy-Studio/{sub.relative_to(ROOT)}")
...
with tarfile.open(mac_tar, "w:gz") as tar:
    for rel_path in TRACKED_PATHS:
        full_p = ROOT / rel_path
        if full_p.exists():
            tar.add(full_p, arcname=f"AI-Privacy-Studio/{rel_path}")
